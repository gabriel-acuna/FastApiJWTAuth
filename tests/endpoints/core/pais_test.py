...
async def test_listar_paises(test_app):
    ...
    resp = await cliente.get('/api/paises', headers= token)
    assert resp.status_code == 200
    assert len(resp.json()) > 0

@pytest.mark.asyncio
async def test_obtener_pais(test_app):
    resp = await test_app[0].get('/api/paises/1', headers= test_app[1])
    assert resp.status_code == 200
    r = resp.json()
...
async def test_obtener_pais_con_status_code_404(test_app):
    ...
    resp = await test_app[0].get('/api/paises/1000', headers= test_app[1])
...
async def test_obtener_pais_con_status_code_422(test_app):
    resp = await test_app[0].get('/api/paises/uji', headers = test_app[1])
