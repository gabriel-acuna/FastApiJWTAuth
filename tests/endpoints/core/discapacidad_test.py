...
async def test_registrar_discapacidad_valores_numericos(test_app):
    resp = await test_app[0].post(url='/discapacidades', json={'discapacidad': 2884848})
...
async def test_registrar_discapacidad_espacios(test_app):
    resp = await test_app[0].post(url='/discapacidades', json={'discapacidad': '         '})
    assert resp.status_code == 422
...
async def test_registrar_discapacidad_menor_a_longitud_minima(test_app):
    resp = await test_app[0].post(url='/discapacidades', json={'discapacidad': 'test'})
    assert resp.status_code == 422
...
async def test_registrar_discapacidad_menor_a_longitud_máxima(test_app):
    resp = await test_app[0].post(url='/discapacidades', json={'discapacidad': 'ucboqsc cpsjqcjqspcpisqhc  ojcosjcoja qpisj    ahjpi   axajxa  hxia    jxi ji xaixjo   akxoa   xhapi   oajxa   oxxjaox oxjaj   x oa    xja xo  aiiiiiiiii'})
    assert resp.status_code == 422
...
async def test_listar_discapacidades(test_app):
    resp = await test_app[0].get(url='/discapacidades')
